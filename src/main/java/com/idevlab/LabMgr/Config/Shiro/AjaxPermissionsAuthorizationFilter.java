...
public class AjaxPermissionsAuthorizationFilter extends FormAuthenticationFilter {
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response) {
		...
		} else // 否则返回错误
		{
			...
			response(request, response, jsonObject);
		}
		return false;
	}

	private void response(ServletRequest request, ServletResponse response, JSONObject error) {
		...
		try {
			...
			res.setHeader("Access-Control-Allow-Headers",
					"Origin, No-Cache, X-Requested-With,Expires,Content-Type, X-E4M-With,userId,token,XX-Token,XX-Device-Type,sessionID");
	...
	@Bean
	public FilterRegistrationBean<AjaxPermissionsAuthorizationFilter> registration(
			AjaxPermissionsAuthorizationFilter filter) {
		FilterRegistrationBean<AjaxPermissionsAuthorizationFilter> registration = new FilterRegistrationBean<AjaxPermissionsAuthorizationFilter>(
				filter);
